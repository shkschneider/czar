...
int32_t main()
{
    _cz_println("Factorial calculations:");
    const int32_t test_values[6] = { 0, 1, 5, 7, 10, 12 };
    int32_t idx = 0;
    while ((idx < 6)) {
        const int32_t n = test_values[idx];
        const int32_t result_rec = factorial_recursive(n);
        const int32_t result_iter = factorial_iterative(n);
        if ((result_rec != result_iter)) {
        _cz_println("ERROR: Mismatch between recursive and iterative!");
        return 1;
    }
        _cz_printf("factorial(%d) = %d\n", n, result_iter);
        (idx = (idx + 1));
    _loop_continue_2: ;
    }
    _loop_break_2: ;
