...
function Memory.gen_memory_tracking_helpers()
    ...
    ctx():emit("static size_t _cz_explicit_alloc_count = 0;")
    ctx():emit("static size_t _cz_explicit_alloc_bytes = 0;")
    ctx():emit("static size_t _cz_implicit_alloc_count = 0;")
    ctx():emit("static size_t _cz_implicit_alloc_bytes = 0;")
    ctx():emit("static size_t _cz_explicit_free_count = 0;")
    ctx():emit("static size_t _cz_implicit_free_count = 0;")
    ctx():emit("static size_t _cz_current_alloc_count = 0;")
    ctx():emit("static size_t _cz_current_alloc_bytes = 0;")
    ctx():emit("static size_t _cz_peak_alloc_count = 0;")
    ctx():emit("static size_t _cz_peak_alloc_bytes = 0;")
    ctx():emit("")
    ctx():emit("void* " .. alloc_prefix .. "_alloc(size_t size, int is_explicit) {")
    ctx():emit("    void* ptr = malloc(size);")
    ctx():emit("    if (ptr) {")
    ctx():emit("        if (is_explicit) {")
    ctx():emit("            _cz_explicit_alloc_count++;")
    ctx():emit("            _cz_explicit_alloc_bytes += size;")
    ctx():emit("        } else {")
    ctx():emit("            _cz_implicit_alloc_count++;")
    ctx():emit("            _cz_implicit_alloc_bytes += size;")
    ctx():emit("        }")
    ctx():emit("        _cz_current_alloc_count++;")
    ctx():emit("        _cz_current_alloc_bytes += size;")
    ctx():emit("        if (_cz_current_alloc_count > _cz_peak_alloc_count) {")
    ctx():emit("            _cz_peak_alloc_count = _cz_current_alloc_count;")
    ctx():emit("        }")
    ctx():emit("        if (_cz_current_alloc_bytes > _cz_peak_alloc_bytes) {")
    ctx():emit("            _cz_peak_alloc_bytes = _cz_current_alloc_bytes;")
    ctx():emit("        }")
    ctx():emit("    }")
    ctx():emit("    return ptr;")
    ctx():emit("}")
    ctx():emit("")
    ctx():emit("void* " .. alloc_prefix .. "_ralloc(void* ptr, size_t new_size) {")
    ctx():emit("    // Note: tracking for ralloc is simplified - doesn't track size changes")
    ...
    ctx():emit("    if (ptr) {")
    ctx():emit("        if (is_explicit) {")
    ctx():emit("            _cz_explicit_free_count++;")
    ctx():emit("        } else {")
    ctx():emit("            _cz_implicit_free_count++;")
    ctx():emit("        }")
    ctx():emit("        _cz_current_alloc_count--;")
    ...
    ctx():emit("}")
    ctx():emit("")
    ctx():emit("void _cz_print_memory_stats(void) {")
    ctx():emit("    size_t total_alloc_count = _cz_explicit_alloc_count + _cz_implicit_alloc_count;")
    ctx():emit("    size_t total_alloc_bytes = _cz_explicit_alloc_bytes + _cz_implicit_alloc_bytes;")
    ctx():emit("    size_t total_free_count = _cz_explicit_free_count + _cz_implicit_free_count;")
