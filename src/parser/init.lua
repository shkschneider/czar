...
function Parser.token_label(tok)
...
function Parser:expect(type_, value)
    ...
    if not self:match(type_, value) then
        error(string.format("expected %s but found %s", value or type_, Parser.token_label(tok)))
    end
    return tok
end
...
function Parser:parse_top_level()
    ...
    elseif self:check("DIRECTIVE") then
        ...
        error(string.format("unexpected token in top-level: %s", Parser.token_label(self:current())))
    end
end
