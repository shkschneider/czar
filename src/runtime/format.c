...
void runtime_emit_format(FILE *output) {
    ...
    fprintf(output, "    }\n");
    fprintf(output, "    \n");
    fprintf(output, "    /* Start with reasonable size, will grow if needed */\n");
    fprintf(output, "    size_t capacity = strlen(fmt) + argc * 64 + 1;\n");
    fprintf(output, "    char *result = malloc(capacity);\n");
    ...
    fprintf(output, "    }\n");
    fprintf(output, "    \n");
    fprintf(output, "    size_t pos = 0;\n");
    fprintf(output, "    int arg_idx = 0;\n");
    fprintf(output, "    const char *p = fmt;\n");
    fprintf(output, "    \n");
    fprintf(output, "    while (*p) {\n");
    fprintf(output, "        /* Ensure we have space (leave room for null terminator and safety margin) */\n");
    fprintf(output, "        if (pos + 128 > capacity) {\n");
    fprintf(output, "            capacity *= 2;\n");
    fprintf(output, "            char *new_result = realloc(result, capacity);\n");
    fprintf(output, "            if (!new_result) {\n");
    fprintf(output, "                result[pos] = '\\0';\n");
    fprintf(output, "                return result;\n");
    fprintf(output, "            }\n");
    fprintf(output, "            result = new_result;\n");
    fprintf(output, "        }\n");
    fprintf(output, "        \n");
    ...
    fprintf(output, "                int written;\n");
    fprintf(output, "                switch (arg.type) {\n");
    fprintf(output, "                    case ANY_INT:\n");
    fprintf(output, "                        written = snprintf(result + pos, capacity - pos, \"%%ld\", arg.v.i);\n");
    fprintf(output, "                        break;\n");
    fprintf(output, "                    case ANY_UINT:\n");
    fprintf(output, "                        written = snprintf(result + pos, capacity - pos, \"%%lu\", arg.v.u);\n");
    fprintf(output, "                        break;\n");
    ...
    fprintf(output, "                        written = snprintf(result + pos, capacity - pos, \"%%zu\", (size_t)arg.v.u);\n");
    fprintf(output, "                        break;\n");
    fprintf(output, "                    case ANY_DOUBLE:\n");
    fprintf(output, "                        written = snprintf(result + pos, capacity - pos, \"%%g\", arg.v.d);\n");
    fprintf(output, "                        break;\n");
    fprintf(output, "                    case ANY_CHAR:\n");
    fprintf(output, "                        result[pos++] = arg.v.c;\n");
    fprintf(output, "                        written = 0;\n");
    fprintf(output, "                        break;\n");
    fprintf(output, "                    case ANY_CSTR:\n");
    fprintf(output, "                        if (arg.v.s) {\n");
    fprintf(output, "                            written = snprintf(result + pos, capacity - pos, \"%%s\", arg.v.s);\n");
    fprintf(output, "                        } else {\n");
    fprintf(output, "                            written = 0;\n");
    fprintf(output, "                        }\n");
    fprintf(output, "                        break;\n");
    fprintf(output, "                    case ANY_PTR:\n");
    fprintf(output, "                        written = snprintf(result + pos, capacity - pos, \"%%p\", arg.v.p);\n");
    fprintf(output, "                        break;\n");
    fprintf(output, "                    default:\n");
    fprintf(output, "                        written = 0;\n");
    fprintf(output, "                        break;\n");
    fprintf(output, "                }\n");
    fprintf(output, "                if (written > 0) pos += written;\n");
    fprintf(output, "            }\n");
    fprintf(output, "            p += 2;\n");
