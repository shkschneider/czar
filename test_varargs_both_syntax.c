...
int32_t sum_go_style(const int32_t* nums, size_t nums_count);
int32_t sum_traditional(const int32_t* nums, size_t nums_count);
uint8_t compute_go(const uint8_t* values, size_t values_count);
uint8_t compute_traditional(const uint8_t* values, size_t values_count);
int32_t main();
...
int32_t main()
{
    const int32_t result1 = sum_go_style((int32_t[]){10, 20, 30}, 3);
    _cz_printf("Go-style result: %d\n", result1);
    const int32_t result2 = sum_traditional((int32_t[]){10, 20, 30}, 3);
    _cz_printf("Traditional result: %d\n", result2);
    const uint8_t result3 = compute_go((uint8_t[]){5, 10}, 2);
    _cz_printf("Go-style u8 result: %d\n", ((int32_t)result3));
    const uint8_t result4 = compute_traditional((uint8_t[]){5, 10}, 2);
    _cz_printf("Traditional u8 result: %d\n", ((int32_t)result4));
    if ((result1 != 60)) {
        return 1;
    }
    if ((result2 != 60)) {
